...
public class NumberUITest : MonoBehaviour
{
    ...
    private void Start()
    {
        SoundManager.Instance.PlaySFX("NumberCount");
        ...
        sq = DOTween.Sequence();
        sq.SetUpdate(true);
        sq.Append(one.rectTransform.DOScale(new Vector2(1,1), 1f));
        sq.Append(two.rectTransform.DOScale(new Vector2(1, 1), 1f))
            .Join(one.DOFade(0,0.6f));
        sq.Append(three.rectTransform.DOScale(new Vector2(1,1), 1f))
            .Join(two.DOFade(0, 0.6f)) ;
        sq.Append(go.rectTransform.DOScale(new Vector2(1,1), 1f).SetEase(Ease.OutCubic))
            .Join(three.DOFade(0, 0.6f));
